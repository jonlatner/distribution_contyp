...
df_graph_period_change_country <- df_graph %>%
        ...
        ungroup() %>%
        mutate(label = "Percentage change",
               direction = ifelse(pct_change > .1, yes = 1, 
                                  ifelse(pct_change < -.1, yes = -1, no = 0)))
...
df_graph_period_change_country <- df_graph_period_change_country %>%
        group_by(country_name) %>%
        mutate(both = ifelse(pct_change > .1 & lag(pct_change,1) > .1, yes = 1, no = 0)) %>%
        ungroup()
table(df_graph_period_change_country$both)
with(df_graph_period_change_country,table(period,direction))

rm(df_graph_period_change_country,df_match,df_graph,df_graph_region,region,df_graph_period_change)

# Graph percent FTC (subgroup/regions) -----------------------------------------
# Gender 
...
ggplot(data=df_graph_period_change, aes(x=period, y=pct_change)) +
        ...
        geom_text(aes(label=sprintf(pct_change, fmt = '%#.3f')), 
                  ...
                  size=3)+
        # theme_bw() +
        geom_rect(data = subset(df_graph_period_change, region %!in% c("Continental", "Southern") | variable %!in% c("< Secondary")), 
                  fill = NA, colour = "gray50", xmin = -Inf,xmax = Inf,
                  ymin = -Inf,ymax = Inf) +
        geom_rect(data = subset(df_graph_period_change, region %in% c("Continental", "Southern") | variable %in% c("< Secondary")), 
                  fill = "green", colour = "green", xmin = -Inf,xmax = Inf,alpha = 0.1,
                  ymin = -Inf,ymax = Inf) +
        theme(panel.grid.minor = element_blank(), 
...
rm(df_graph_region,df_graph_period_change)

# Graph percent FTC (subgroup/continental) -----------------------------------------

df_graph_continental <- df_graph %>%
        filter(region == "Continental")

df_graph_continental <- df_graph_continental %>%
        mutate(levels = ifelse(country_name == "Netherlands", yes = "Netherlands",
                               ifelse(country_name == "France", yes = "France", 
                                      ifelse(country_name == "Germany", yes = "Germany",
                                             ifelse(level == "Countries", yes = "Countries",
                                                    ifelse(level == "Region", yes = "Region",
                                                           no = NA))))))

df_graph_continental$levels <- fct_relevel(df_graph_continental$levels, "Countries", after = 0) # forcats
df_graph_continental$levels <- fct_relevel(df_graph_continental$levels, "Region", after = 0) # forcats

df_graph_region <- df_graph_continental %>%
        filter(level != "Countries")

ggplot(df_graph_continental, aes(x = year, y = avg, group = country_name, color = levels, size = levels)) +
        facet_grid(period ~ variable) +
        scale_size_manual(values = c(1,.25,.75,.75, .75)) +
        scale_color_manual(values = c("black", "gray", "blue", "darkred", "orange")) +
        geom_line() +
        scale_x_continuous(breaks = c(1,6,12), limits = c(-.5,13.5)) +
        theme_bw() +
        theme(panel.grid.minor = element_blank(), 
...
ggsave(filename = paste0(graphs,"graph_rate_continental_presentation.pdf"), height = 4, width = 8, units = "in")
...
ggsave(filename = paste0(graphs,"graph_pct_change_continental_presentation.pdf"), height = 2, width = 8, units = "in")

# Graph percent FTC (subgroup/southern) -----------------------------------------

df_graph_southern <- df_graph %>%
        filter(region == "Southern")

df_graph_southern$level <- factor(df_graph_southern$level,
                         levels = c("Region", "Countries"))

df_graph_southern <- df_graph_southern %>%
        mutate(levels = ifelse(country_name == "Italy", yes = "Italy",
                               ifelse(country_name == "Portugal", yes = "Portugal",
                                      ifelse(country_name == "Spain", yes = "Spain",
                                             ifelse(level == "Countries", yes = "Countries",
...
df_graph_southern$levels <- fct_relevel(df_graph_southern$levels, "Countries", after = 0) # forcats
df_graph_southern$levels <- fct_relevel(df_graph_southern$levels, "Region", after = 0) # forcats

df_graph_region <- df_graph_southern %>%
        filter(level != "Countries")

ann_text <- tribble(
        ~period, ~region, ~year, ~avg, ~country_name,~levels, ~factor,~variable,
        "2007 - 2018", "Southern", 3.3, .42, "Cyprus", "Countries", "Education","< Secondary"
)

ann_text$variable <- factor(ann_text$variable,
                                  levels = c("Male", "Female", "< 35", "35 - 45", "> 45", "< Secondary", "Secondary", "> Secondary"))

ggplot(df_graph_southern, aes(x = year, y = avg, group = country_name, color = levels, size = levels)) +
        facet_grid(period ~ variable) +
        scale_size_manual(values = c(1,.25,.75,.75,.75)) +
        scale_color_manual(values = c("black", "gray", "darkgreen", "purple", "gold")) +
        geom_line() +
        scale_x_continuous(breaks = c(1,6,12), limits = c(-.5,13.5)) +
        ...
        geom_text(data = ann_text, label = "Cyprus", size = 2.5, show.legend = FALSE)
...
ggsave(filename = paste0(graphs,"graph_rate_southern_presentation.pdf"), height = 4, width = 8, units = "in")
...
ggplot(data=df_graph_period_change, aes(x=period, y=pct_change)) +
        facet_grid(label ~ variable) +
        scale_y_continuous(breaks = seq(0,1,.25), limits = c(-.1,1)) +
        geom_bar(stat="identity") +
        geom_text(aes(label=sprintf(pct_change, fmt = '%#.3f')), 
                  vjust=ifelse(df_graph_period_change$pct_change>0, yes = -1, no = 1.1), 
                  size=2.5)+
...
ggsave(filename = paste0(graphs,"graph_pct_change_southern_presentation.pdf"), height = 2, width = 8, units = "in")

# Graph percent FTC (< secondary/region) -----------------------------------------

df_graph_lhs <- df_graph %>%
        filter(variable == "< Secondary")

# df_graph_lhs$level <- factor(df_graph_lhs$level,
#                              levels = c("Region", "Countries"))

df_graph_lhs <- df_graph_lhs %>%
        mutate(levels = ifelse(country_name == "Italy", yes = "Italy",
                               ifelse(country_name == "Poland", yes = "Poland",
                                      ifelse(country_name == "Netherlands", yes = "Netherlands",
                                             ifelse(country_name == "France", yes = "France", 
                                                    ifelse(country_name == "Portugal", yes = "Portugal", 
                                                           ifelse(country_name == "Spain", yes = "Spain", 
                                                                  ifelse(country_name == "Hungary", yes = "Hungary", 
                                                                                ifelse(country_name == "Sweden", yes = "Sweden",
                                                                                       ifelse(level == "Countries", yes = "Countries",
                                                                                              ifelse(level == "Region", yes = "Region",
                                                                                                     no = NA)))))))))))

df_graph_lhs$levels <- factor(df_graph_lhs$levels,
                             levels = c("Region", "Countries",
                                        "France", "Netherlands",
                                        "Poland", "Hungary", 
                                        "Sweden",
...
# df_graph_lhs$levels <- fct_relevel(df_graph_lhs$levels, "Countries", after = 0) # forcats
# df_graph_lhs$levels <- fct_relevel(df_graph_lhs$levels, "Region", after = 0) # forcats

# df_graph_region <- df_graph_lhs %>%
#         filter(level != "Countries")

ann_text <- tribble(
        ~period, ~region, ~year, ~avg, ~country_name,~levels,
        "2007 - 2018", "Eastern", 6, .6, "Slovakia", "Countries"
)

ann_text$region <- fct_relevel(ann_text$region, "Europe", after = 0) # forcats

ggplot(df_graph_lhs, aes(x = year, y = avg, group = country_name, color = levels, size = levels)) +
        facet_grid(period ~ region) +
        # scale_size_manual(values = c(1,.25)) +
        # scale_color_manual(values = c("black", "gray")) +
        scale_size_manual(values = c(1,.25,.5,.5,.5,.5,.5,.5,.5,.5)) +
        scale_color_manual(values = c("black", "gray", "blue", "orange", "darkred", "green", "lightblue", "darkgreen", "purple", "gold")) +
        geom_line() +
        scale_x_continuous(breaks = c(1,6,12), limits = c(-.5,13.5)) +
        theme_bw() +
        ...
        geom_text(data = ann_text, label = "Slovakia", size = 3, show.legend = FALSE)
...
ggsave(filename = paste0(graphs,"graph_rate_lhs_presentation.pdf"), height = 4, width = 8, units = "in")

# View(filter(df_graph_lhs, period == "2007 - 2018" & region == "Eastern" & avg > .4))
# View(filter(df_graph_lhs, period == "2007 - 2018" & region == "Nordic" & avg > .2))
...
ggsave(filename = paste0(graphs,"graph_pct_change_lhs_presentation.pdf"), height = 2, width = 8, units = "in")
