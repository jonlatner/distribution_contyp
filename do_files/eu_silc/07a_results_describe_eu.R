...
df_ever <- readRDS(file = paste0(results,"df_yhat_glm_ever_eu_wt.rds"))
df_num <- readRDS(file = paste0(results,"df_yhat_glm_num_eu_wt.rds"))
df_dur <- readRDS(file = paste0(results,"df_yhat_glm_dur_eu_wt.rds"))

# Clean ever temporary ----

df_ever_2008_2019 <- df_ever %>%
        select(panel,fit,lwr,upr) %>%
