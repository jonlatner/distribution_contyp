...
for(c in country) {
  ...
  # do not run models in country, years where models do not run do to low number of observations
  t <- t %>%
    mutate(test = ifelse((country == "Denmark" & (panel == 2014 | panel == 2018)) 
                         | (country == "Iceland" & (panel == 2015)) 
                         | (country == "Estonia" & (panel == 2008|panel == 2013|panel == 2015|panel == 2019)) 
                         | (country == "Ireland" & (panel == 2014|panel == 2015)) 
                         | (country == "Latvia" & (panel == 2015|panel == 2017|panel==2019)) 
                         | (country == "Lithuania" & (panel == 2018)) 
                         | (country == "Romania" & (panel == 2015))
  ...
  for(p in panel) {
    ...
    my_svy <- svydesign(data = df_panel, 
                        ids = ~1, # no clusters 
                        weights =~ weight_long)
    beta <- svyglm(ftc_num ~ male + age_cat + edu_cat, my_svy, family = "binomial")
...
rm(beta,table,yhat,mfx,df_panel,df_new_data_yhat,p,panel,my_svy,df_country,country,c,t)
...
df_eu_silc <- df_eu_silc %>%
  ...
  mutate(ftc_c = ftc_c - 1)

table(df_eu_silc$ftc_c, useNA = "ifany")
...
=======
  ...
  beta <- svyglm(ftc_c ~ male + age_cat + edu_cat, my_svy, family = "binomial")
...
saveRDS(df_yhat_country, file = paste0(results, "df_yhat_glm_num_country_wt.rds"))

saveRDS(df_table_eu, file = paste0(results, "df_table_glm_num_eu_wt.rds"))
saveRDS(df_table_region, file = paste0(results, "df_table_glm_num_region_wt.rds"))
saveRDS(df_table_country, file = paste0(results, "df_table_glm_num_country_wt.rds"))

saveRDS(df_mfx_eu, file = paste0(results, "df_mfx_glm_num_eu_wt.rds"))
saveRDS(df_mfx_region, file = paste0(results, "df_mfx_glm_num_region_wt.rds"))
saveRDS(df_mfx_country, file = paste0(results, "df_mfx_glm_num_country_wt.rds"))
=======
rm(beta,table,yhat,mfx,df_panel,df_new_data_yhat,p,panel,my_svy)
...
for(c in country) {
  ...
  t <- data.frame(with(df_country,table(panel,ftc_c)))
  t$country <- c
  
  # do not run models in country, years where models do not run do to low number of observations
  t <- t %>%
    mutate(test = ifelse((country == "Denmark" & (panel == 2014 | panel == 2018))
                         | (country == "Estonia" & (panel == 2008|panel == 2019))
                         | (country == "Ireland" & (panel == 2014|panel == 2015))
                         | (country == "Latvia" & (panel == 2015|panel == 2017|panel==2019))
                         | (country == "Lithuania" & (panel == 2018))
                         | (country == "Romania" & (panel == 2015))
...
saveRDS(df_yhat_eu, file = paste0(results, "df_yhat_glm_contract_num_eu_wt.rds"))
saveRDS(df_yhat_region, file = paste0(results, "df_yhat_glm_contract_num_region_wt.rds"))
saveRDS(df_yhat_country, file = paste0(results, "df_yhat_glm_contract_num_country_wt.rds"))

saveRDS(df_table_eu, file = paste0(results, "df_table_glm_contract_num_eu_wt.rds"))
saveRDS(df_table_region, file = paste0(results, "df_table_glm_contract_num_region_wt.rds"))
saveRDS(df_table_country, file = paste0(results, "df_table_glm_contract_num_country_wt.rds"))

saveRDS(df_mfx_eu, file = paste0(results, "df_mfx_glm_contract_num_eu_wt.rds"))
saveRDS(df_mfx_region, file = paste0(results, "df_mfx_glm_contract_num_region_wt.rds"))
saveRDS(df_mfx_country, file = paste0(results, "df_mfx_glm_contract_num_country_wt.rds"))
>>>>>>> c13cc683d58889aaba8f5471ecad5c7a881d390e
