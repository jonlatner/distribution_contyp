...
tables = "SECCOPA/projects/distribution_contyp/tables/2_year_panel/"
...
df_filter <- df_filter %>%
  ...
  summarise(country_panel = max(row_number()),
            total_obs = sum(n),
            total_unique_obs = sum(unique_n)) %>%
  ungroup() %>%
  mutate(obs_diff = paste0(round((total_obs/lag(total_obs,1)-1)*100,0),"\\%"),
       unique_obs_diff = paste0(round((total_unique_obs/lag(total_unique_obs,1)-1)*100,0),"\\%"),
       obs_diff = ifelse(row_number()==1, yes = "", no = obs_diff),
       unique_obs_diff = ifelse(row_number()==1, yes = "", no = unique_obs_diff))
head(df_filter)
                                     ...
                                     ifelse(step == 2, yes = "Individual filters: prime age (25 - 54), active labor market participation (employed or unemployed), personal weight $>$ 0",
                                            ifelse(step == 3, yes = "Must be employed at least once",
                                                   ifelse(step == 4, yes = "Case-wise deletion of missing variables on education, gender, age, and contract type",
                                                          ifelse(step == 5, yes = "Individuals in each year of 2 year panel period",
                                                                 ifelse(step == 6, yes = "Must have 2 year longitudinal weight (`Main dataset')", no = NA))))))))


# Table -----------------------------------------

df_table <- df_filter %>%
  select(step,country_panel,total_obs,obs_diff,notes)
