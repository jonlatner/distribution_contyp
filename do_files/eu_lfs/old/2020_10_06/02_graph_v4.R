...
p <- ggplot(df_graph, aes(x = year, y = avg, group = country_name, color = level, size = level)) +
        ...
        geom_line() +
        scale_x_continuous(breaks = c(1996,2000,2007,2012,2018), limits = c(1995,2018)) +
        ...
        geom_text(data = df_graph_region,show.legend = FALSE,
                  size = 2.5, 
                  aes(x = year, y = ifelse(year %in% c(1996,2000,2007,2012,2018), yes = avg, no = NA),
                      vjust=-1,
                      label=sprintf(avg, fmt = '%#.2f')))
...
p <- ggplot(df_graph, aes(x = year, y = avg, group = country_name, color = level, size = level)) +
        ...
        scale_color_manual(values = c("black", "gray")) +
        geom_line() +
        scale_x_continuous(breaks = c(1996, 2001, 2006), limits = c(1995, 2007)) +
        ...
        geom_text(data = df_graph_region,
                  show.legend = FALSE,
                  size = 2, 
                  aes(x = year, y = ifelse(year %in% c(1996, 2001, 2006), yes = avg, no = NA),
                      vjust=-1,
                      label=sprintf(avg, fmt = '%#.2f')))
...
p <- ggplot(df_graph, aes(x = year, y = avg, group = country_name, color = level, size = level)) +
        ...
        scale_color_manual(values = c("black", "gray")) +
        geom_line() +
        scale_x_continuous(breaks = c(2007, 2012, 2018), limits = c(2006, 2018)) +
        # scale_y_continuous(breaks = seq(0, .5, by = .1), limits = c(0, .5)) +
        ylab("Temporary employment as % of total emp (25-54)") +
        xlab("Period") + 
        theme_bw() +
        ...
        geom_text(data = df_graph_region,
                  show.legend = FALSE,
                  size = 2, 
                  aes(x = year, y = ifelse(year %in% c(2007,2012,2018), yes = avg, no = NA),
                      vjust=-1,
                      label=sprintf(avg, fmt = '%#.2f')))
