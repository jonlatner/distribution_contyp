...
df_graph_eu <- df_eu_lfs %>%
        ...
        mutate(country_name = "Europe", 
               country = "EU",
               period = ifelse(year<2007, yes = 0, no = 1),
               year2 = ifelse(period == 0, 
                              yes = year - 1995,
                              no = year - 2006))
...
df_graph <- df_graph %>%
        mutate(highlight_flag = ifelse(country_name == "Europe", yes = "Europe", "Countries"),
...
ggplot() +
        geom_line(aes(x = year2, y = avg, group = country_name, color = highlight_flag, size = highlight_flag), data = df_graph) +
        facet_grid(. ~ period) +
