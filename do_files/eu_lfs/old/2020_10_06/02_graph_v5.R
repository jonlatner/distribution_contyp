...
p <- ggplot(df_graph, aes(x = year, y = avg, group = country_name, color = level, size = level)) +
        ...
        scale_color_manual(values = c("black", "gray")) +
        geom_line() +
        scale_x_continuous(breaks = c(1996, 2001, 2007), limits = c(1995, 2008)) +
        ylab("Temporary employment as % of total emp (25-54)") +
        xlab("Period") + 
        theme_bw() +
        theme(panel.grid.minor = element_blank(), 
        ...
        geom_text(data = df_graph_region,
                  show.legend = FALSE,
                  size = 2, 
                  aes(x = year, y = ifelse(year %in% c(1996, 2001, 2007), yes = avg, no = NA),
                      vjust=-1,
                      label=round(avg, 3)))
...
p <- ggplot(df_graph, aes(x = year, y = avg, group = country_name, color = level, size = level)) +
        ...
        scale_color_manual(values = c("black", "gray")) +
        geom_line() +
        scale_x_continuous(breaks = c(2007, 2012, 2018), limits = c(2006, 2019)) +
