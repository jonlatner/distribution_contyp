...
graphs = "SECCOPA/projects/distribution_contyp/graphs/age_25_65/"
...
df_eu_lfs_1$age_cat <- recode(df_eu_lfs_1$age, "25:34 = 1; 35:49=2; 50:hi=3")
df_eu_lfs_1$age_cat <- factor(df_eu_lfs_1$age_cat, labels=c("< 35", "35-49", "> 50"))
...
p <- ggplot(df_graph, aes(x = year, y = avg, group = country_name, color = level, size = level)) +
        ...
        ylab("Temporary employment as % of total emp (25-64)") +
        xlab("Period") + 
        theme_bw() +
