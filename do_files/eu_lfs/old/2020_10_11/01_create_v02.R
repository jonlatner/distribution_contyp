...
year <- seq(1998,2019,1)
for(c in seq_along(country)) {
        test <- data.frame()
        print(country[c])
        for(yr in seq_along(year)) {
                print(year[yr])
                df <- read.csv(file = paste0(orig_data_files,"/YearlyFiles_1998_2019/",country[c],"_YEAR_1998_onwards/",country[c],year[yr],"_y.csv"))
                df <- select(df, TEMP, AGE, SEX, MARSTAT, COUNTRYB, ILOSTAT, FTPT, HATLEV1D, COEFF)
                df$year <- year[yr]
                df$country <- country[c]
                test <- rbind(test,df)
