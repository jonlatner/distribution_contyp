...
# Graph percent by country/region and period -----------------------------------------

df_period_1 <- df_eu_lfs_0 %>%
        filter(year <= 2007) %>%
        mutate(period = 1,
               year = year - 1995)

df_period_2 <- df_eu_lfs_0 %>%
        filter(year >= 2007) %>%
        mutate(period = 2,
               year = year - 2006)

df_graph_0 <- rbind(df_period_1,df_period_2)
rm(df_period_1,df_period_2)

df_graph_0$period <- factor(df_graph_0$period,
                          labels = c("1996 - 2007", "2007 - 2018"))

df_graph_1 <- df_graph_0 %>%
        group_by(period,year,country_name) %>%
...
df_test <- df_graph_1 %>%
        ...
        mutate(test = ifelse((pct_change>.3 & row_number()==1), yes = 1, no = 0),
               test = ifelse((pct_change>.3 & row_number()==2), yes = 1, no = test),
               test = sum(test)) %>%
        ungroup()%>%
        filter(test == 2) %>%
...
df_country_pct <- df_eu_lfs_0 %>%
        filter(country_name == "Italy" | country_name == "Netherlands" | country_name == "Germany" | country_name == "United Kingdom") %>%
...
df_graph_countries <- df_eu_lfs_0 %>%
